...
namespace HW_10
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            fs.SetLength(0);
            fs.Close();
            Event event1 = new Event("День первокурсника", new DateTime(2023, 12, 10), group321, 1);
            event1.Select();
            event1.Print();
            Event event2 = new Event("Гала-концерт", new DateTime(2023, 12, 10), group322, 2);
            event2.Select();
            event2.Print();
            Event event3 = new Event("МиМ ИВМиИТ", new DateTime(2023, 12, 10), group321, 3);
            event3.Select();
            event3.Print();
